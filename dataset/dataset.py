...
class Dataset(data.Dataset):
    def __init__(
        ...
        split_info = None
        split_classes = None
        split_img_paths = None
        if source_split_path is not None and source_split_role is not None:
            split_info = json.load(open(source_split_path, 'r'))
            if source_split_role not in ['train', 'val']:
                raise ValueError("source_split_role must be train or val, got {}".format(source_split_role))
            split_classes = set(split_info.get(f'{source_split_role}_classes', []))
            split_img_paths = set(split_info.get(f'{source_split_role}_img_paths', []))
        ...
        if split_classes:
            self.cls_names = [cls_name for cls_name in self.cls_names if cls_name in split_classes]
        ...
        if split_img_paths:
            self.data_all = [item for item in self.data_all if item['img_path'] in split_img_paths]

        self.length = len(self.data_all)
        if self.length == 0:
            raise ValueError(
                "Dataset split is empty: root={}, role={}, split={}".format(
                    self.root,
                    source_split_role,
                    source_split_path,
                )
