...
class STARCLIPBackbone(nn.Module):
    def __init__(self,
                 embed_dim: int,
                 # vision
...
def compute_similarity_batchwise(image_features, text_features):
    """Cosine-similarity map with per-sample text anchors.

    Args:
        image_features: [B, N, D] normalized patch tokens.
        text_features: [B, 2, D] normalized normal/abnormal anchors.

    Returns:
        similarity probabilities [B, N, 2] and cls-token logits [B, 1, 2].
    """
    similarity = torch.einsum("bnd,btd->bnt", image_features, text_features)
    prob_1 = similarity[:, :1, :]
    return (similarity / 0.07).softmax(-1), prob_1
...
def _gaussian_kernel2d(kernel_size, sigma, device, dtype):
    if kernel_size <= 1:
        return None
    if kernel_size % 2 == 0:
        raise ValueError("Gaussian patch aggregation kernel_size must be odd, got {}".format(kernel_size))
...
def _aggregate_patch_grid(patch_tokens, kernel_size, sigma):
    if kernel_size <= 1:
        return patch_tokens
    bsz, num_tokens, dim = patch_tokens.shape
    grid_h, grid_w = _infer_square_grid(num_tokens)
    patch_grid = patch_tokens.transpose(1, 2).reshape(bsz, dim, grid_h, grid_w)
    kernel = _gaussian_kernel2d(kernel_size, sigma, patch_tokens.device, patch_tokens.dtype)
    depthwise_kernel = kernel.view(1, 1, kernel_size, kernel_size).repeat(dim, 1, 1, 1)
    pad = kernel_size // 2
    patch_grid = F.pad(patch_grid, (pad, pad, pad, pad), mode="replicate")
    aggregated = F.conv2d(patch_grid, depthwise_kernel, groups=dim)
    return aggregated.reshape(bsz, dim, num_tokens).transpose(1, 2)


def aggregate_patch_tokens_multiscale(
    patch_feature,
    enabled=False,
    kernel_sizes=(1, 3, 5),
    sigma=4.0,
    ...
    """Multi-scale Gaussian neighborhood aggregation over patch tokens.

    The CLS token, when present, is kept unchanged. Downstream similarity code
    still normalizes the resulting features, so this only injects local spatial
    context before patch-anchor matching.
    """
    if not enabled:
        return patch_feature
    if patch_feature.ndim != 3:
        raise ValueError("patch_feature must be [B, N, D], got {}".format(tuple(patch_feature.shape)))
    kernel_sizes = tuple(int(k) for k in kernel_sizes)
    if len(kernel_sizes) == 0:
        raise ValueError("patch_ms_kernel_sizes must not be empty")
    if any(k <= 0 for k in kernel_sizes):
        raise ValueError("patch_ms_kernel_sizes must be positive, got {}".format(kernel_sizes))

    num_tokens = patch_feature.shape[1]
    has_cls = False
    if num_tokens > 1:
        side = int(round(math.sqrt(float(num_tokens - 1))))
        has_cls = side * side == num_tokens - 1
    if has_cls:
        cls_token = patch_feature[:, :1, :]
        patch_tokens = patch_feature[:, 1:, :]
    else:
        cls_token = None
        patch_tokens = patch_feature
        _infer_square_grid(patch_tokens.shape[1])

    base_tokens = patch_tokens
    if fuse == "mean":
        aggregated_tokens = [
            _aggregate_patch_grid(base_tokens, kernel_size=k, sigma=sigma)
            for k in kernel_sizes
        ]
        fused_tokens = torch.stack(aggregated_tokens, dim=0).mean(dim=0)
    elif fuse == "residual":
        ms_kernel_sizes = [k for k in kernel_sizes if k > 1]
        if len(ms_kernel_sizes) == 0:
            fused_tokens = base_tokens
        else:
            ms_tokens = [
                _aggregate_patch_grid(base_tokens, kernel_size=k, sigma=sigma)
                for k in ms_kernel_sizes
            ]
            ms_tokens = torch.stack(ms_tokens, dim=0).mean(dim=0)
            fused_tokens = base_tokens + float(residual_beta) * ms_tokens
    else:
        raise ValueError("unsupported patch_ms_fuse: {}".format(fuse))
...
def compute_global_local_score_batchwise(
    query_feats,
    query_patch_feats,
    batch_text_features,
    img_size,
    ...
    """Compute global logits and local map with image-specific anchors.

    This mirrors TextualAdapter.compute_global_local_score but accepts text
    features shaped [B, 2, D], so each image uses its own updated anchors.
    """
    batch_text_features = batch_text_features / batch_text_features.norm(dim=-1, keepdim=True).clamp_min(1e-6)
    query_feats = query_feats / query_feats.norm(dim=-1, keepdim=True).clamp_min(1e-6)

    global_score = torch.einsum("bd,btd->bt", query_feats, batch_text_features) / 0.07

    patch_feature = query_patch_feats[-1]
    patch_feature = aggregate_patch_tokens_multiscale(
        patch_feature,
        enabled=patch_ms_agg,
        kernel_sizes=patch_ms_kernel_sizes,
        sigma=patch_ms_sigma,
        fuse=patch_ms_fuse,
        residual_beta=patch_ms_residual_beta,
    )
    patch_feature = patch_feature / patch_feature.norm(dim=-1, keepdim=True).clamp_min(1e-6)
    similarity, _ = compute_similarity_batchwise(patch_feature, batch_text_features)
    local_score = get_similarity_map(similarity[:, 1:, :], img_size).permute(0, 3, 1, 2)
    return global_score, local_score


class VisionConditionedAnchorUpdater(nn.Module):
    """Predicts per-image residual updates for adaptive text anchors."""

    def __init__(self, input_dim=768, hidden_dim=512, gamma=0.05):
        super().__init__()
        self.gamma = gamma
        self.normal_anchor_updater = nn.Sequential(
            nn.Linear(input_dim, hidden_dim),
            nn.GELU(),
            nn.Linear(hidden_dim, input_dim),
        )
        self.abnormal_anchor_updater = nn.Sequential(
            nn.Linear(input_dim, hidden_dim),
            nn.GELU(),
            nn.Linear(hidden_dim, input_dim),
        )

    def forward(self, query_global_feat, adaptive_text_features):
        query_global_feat = query_global_feat.float()
        adaptive_text_features = adaptive_text_features.float()
        adaptive_text_features = F.normalize(adaptive_text_features, dim=-1)

        adaptive_normal = adaptive_text_features[0]
        adaptive_abnormal = adaptive_text_features[1]
        delta_normal = self.normal_anchor_updater(query_global_feat)
        delta_abnormal = self.abnormal_anchor_updater(query_global_feat)

        updated_normal = F.normalize(
            adaptive_normal.unsqueeze(0) + self.gamma * delta_normal,
            dim=-1,
        )
        updated_abnormal = F.normalize(
            adaptive_abnormal.unsqueeze(0) + self.gamma * delta_abnormal,
            dim=-1,
        )
        updated_text_features = torch.stack([updated_normal, updated_abnormal], dim=1)
        return updated_text_features, delta_normal, delta_abnormal
...
class VisualResidualAdapter(nn.Module):
    def __init__(self, dim, bottleneck_ratio=4, residual_scale=1.0):
        super().__init__()
        hidden_dim = max(dim // max(int(bottleneck_ratio), 1), 1)
...
class VisualAdapter(nn.Module):
    def __init__(
        ...
        reduction=4,
        layers_num=1,
        use_visual_lsar=False,
        visual_lsar_bottleneck_ratio=4,
        visual_lsar_scale=1.0,
    ):
        super().__init__()
        ...
        self.use_visual_lsar = use_visual_lsar

        self.local_adater = ResMLP(input_dim, reduction=reduction)
        self.global_adapter = ResMLP(input_dim, reduction=reduction)
        self.local_lsar_residual = None
        self.global_lsar_residual = None
        if use_visual_lsar:
            self.local_lsar_residual = VisualResidualAdapter(
                input_dim,
                bottleneck_ratio=visual_lsar_bottleneck_ratio,
                residual_scale=visual_lsar_scale,
            )
            self.global_lsar_residual = VisualResidualAdapter(
                input_dim,
                bottleneck_ratio=visual_lsar_bottleneck_ratio,
                residual_scale=visual_lsar_scale,
            )

    def visual_lsar_parameters(self):
        params = []
        if self.local_lsar_residual is not None:
            params += list(self.local_lsar_residual.parameters())
        if self.global_lsar_residual is not None:
            params += list(self.global_lsar_residual.parameters())
        return params

    def set_visual_lsar_requires_grad(self, requires_grad=True):
        for param in self.visual_lsar_parameters():
    ...
    def forward(self, image_features, patch_features, static_text_features):
        ...
        if self.global_lsar_residual is not None:
            image_feature = self.global_lsar_residual(image_feature)
        if self.local_lsar_residual is not None:
            patch_feature = self.local_lsar_residual(patch_feature)
...
class TextualAdapter(nn.Module):
    ...
    def compute_global_local_score(
        self,
        query_feats,
        query_patch_feats,
        text_features,
